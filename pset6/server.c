...
void stop(void);
char* extract(char* Line,char* token);
char* modify(char* line,char* token);
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            // TODO: validate request-line
             ...
             {
                absolute_path = extract(request_target,"?");
                Query = modify(request_target,"?");
             }
            ...
            else if(strchr(request_target,'\"') != NULL)
            {
                error(400);
                continue;
            }
            else if(strcmp(http_ver,"HTTP/1.1\r\n") != 0)
            {
                error(505);
                continue;
            }
